    ...
    protected void call() {
        ...
        for (ClusterEntity clusterEntity : clusterList) {
            ...
            for (VolumeEntity volume : volumes) {
                if (!volume.getVolumeStatus().equals(com.roamblue.cloud.management.util.VolumeStatus.READY)) {
                    continue;
                }
                lockService.tryRun(LockKeyUtil.getVolumeLockKey(volume.getId()), () -> {
                    ...
                    if (storage != null) {
                        ...
                        if (volumeResultUtil.getCode() == ErrorCode.SUCCESS) {
                            ...
                            log.error("检测磁盘错误,磁盘ID={} 存储ID={} 检测主机={} 错误信息={}", volume.getId(), storage.getId(), host.getHostName(), volumeResultUtil.getMessage());
                        }
                    }
                    return null;
                }, 1, TimeUnit.MINUTES);
