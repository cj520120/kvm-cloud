...
import cn.roamblue.cloud.management.operate.bean.SyncHostGuestOperate;
    ...
    public void onFinish(SyncHostGuestOperate param, ResultUtil<List<GuestInfo>> resultUtil) {
        ...
        if (!guestNames.isEmpty()) {
            ...
            for (String guestName : guestNames) {
                ...
                if (!Objects.equals(guest.getHostId(), param.getHostId()) || Objects.equals(cn.roamblue.cloud.management.util.Constant.GuestStatus.STOP, guest.getStatus())) {
                    //不是同一台宿主机或者当前是关机状态
        ...
        //检查运行的客户机不在当前主机
        {
            ...
            for (GuestEntity guest : guestEntityList) {
                if (Objects.equals(guest.getStatus(), cn.roamblue.cloud.management.util.Constant.GuestStatus.RUNNING)
                        && System.currentTimeMillis() - guest.getLastStartTime().getTime() > TimeUnit.MINUTES.toMillis(3)) {
                    //上次超过1分钟，则开始检测
                    if (!guestNames.contains(guest.getName())) {
                        //无效的主机状态，开始自动关机
                        guest.setStatus(cn.roamblue.cloud.management.util.Constant.GuestStatus.STOPPING);
