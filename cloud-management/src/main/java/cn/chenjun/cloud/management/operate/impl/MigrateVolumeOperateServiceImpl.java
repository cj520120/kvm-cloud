...
@Component
@Slf4j
public class MigrateVolumeOperateServiceImpl extends AbstractOperateService<MigrateVolumeOperate, ResultUtil<VolumeInfo>> {


    @Override
    public void operate(MigrateVolumeOperate param) {
        VolumeEntity sourceVolume = volumeDao.findById(param.getSourceVolumeId());
        if (sourceVolume.getStatus() == Constant.VolumeStatus.MIGRATE) {
            StorageEntity sourceStorage = storageDao.findById(sourceVolume.getStorageId());
            ...
            HostEntity host = this.allocateService.allocateHost(HostRole.NONE, 0, null, Math.max(sourceVolume.getHostId(), targetVolume.getHostId()), 0, 0);
