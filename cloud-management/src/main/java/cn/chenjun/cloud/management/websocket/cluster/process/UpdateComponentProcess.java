...
public class UpdateComponentProcess extends AbstractClusterMessageProcess {
    ...
    public void process(NotifyData<?> msg) {
        ...
        if (entity != null) {
            model = ComponentModel.builder().componentId(entity.getComponentId())
        ...
        ResultUtil<ComponentModel> resultUtil = ResultUtil.success(model);
        NotifyData<ResultUtil<ComponentModel>> sendMsg = NotifyData.<ResultUtil<ComponentModel>>builder().id(msg.getId()).type(Constant.NotifyType.UPDATE_DNS).data(resultUtil).build();
