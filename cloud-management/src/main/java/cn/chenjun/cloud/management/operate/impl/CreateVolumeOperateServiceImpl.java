...
@Component
@Slf4j
public class CreateVolumeOperateServiceImpl<T extends CreateVolumeOperate> extends AbstractOperateService<T, ResultUtil<VolumeInfo>> {
        ...
        if (volume.getStatus() == Constant.VolumeStatus.CREATING) {
            if(!Objects.equals(volume.getDevice(), Constant.DeviceType.DISK)){
                ...
                if (param.getTemplateId() > 0) {
                    List<TemplateVolumeEntity> templateVolumeList = templateVolumeDao.listByTemplateId(param.getTemplateId());
                    ...
                    StorageEntity parentStorage = storageDao.findById(templateVolume.getStorageId());
