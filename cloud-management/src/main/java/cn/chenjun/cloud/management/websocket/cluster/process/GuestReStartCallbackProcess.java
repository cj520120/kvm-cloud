...
@Component
public class GuestReStartCallbackProcess extends AbstractClusterMessageProcess {
    @Autowired
    ...
    public void process(NotifyData<?> msg) {
        NotifyData<?> sendMsg = NotifyData.builder().id(msg.getId()).type(Constant.NotifyType.GUEST_RESTART_CALLBACK_NOTIFY).data(msg.getData()).version(System.currentTimeMillis()).build();
