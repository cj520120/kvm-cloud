...
@Component
@Slf4j
public class MigrateTemplateVolumeOperateServiceImpl extends AbstractOperateService<MigrateTemplateVolumeOperate, ResultUtil<VolumeInfo>> {


    @Override
    public void operate(MigrateTemplateVolumeOperate param) {
        TemplateVolumeEntity volume = templateVolumeDao.findById(param.getSourceTemplateVolumeId());
        if (Objects.equals(volume.getStatus(), Constant.TemplateStatus.MIGRATE)) {
            ...
            HostEntity host = this.allocateService.allocateHost(HostRole.NONE, 0, null, sourceStorage.getHostId(), 0, 0);
    ...
    public void onFinish(MigrateTemplateVolumeOperate param, ResultUtil<VolumeInfo> resultUtil) {
        TemplateVolumeEntity sourceVolume = this.templateVolumeDao.findById(param.getSourceTemplateVolumeId());
        TemplateVolumeEntity targetVolume = templateVolumeDao.findById(param.getTargetTemplateVolumeId());
        ...
        if (resultUtil.getCode() == ErrorCode.SUCCESS) {
            ...
            templateVolumeDao.update(targetVolume);

            sourceVolume.setStatus(Constant.TemplateStatus.DESTROY);
            templateVolumeDao.update(sourceVolume);
