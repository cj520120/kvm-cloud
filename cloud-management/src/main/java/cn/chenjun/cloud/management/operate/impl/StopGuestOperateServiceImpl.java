...
@Component
@Slf4j
public class StopGuestOperateServiceImpl extends AbstractOperateService<StopGuestOperate, ResultUtil<Void>> {
    ...
    public void operate(StopGuestOperate param) {
        ...
        if (host == null) {
            ...
            if (!param.isForce()) {
                ...
                long expire = TimeUnit.MINUTES.toMillis(Math.max(maxWaitMinutes, 1));
                GuestShutdownRequest request = GuestShutdownRequest.builder().name(guest.getName()).expireMillis(expire).build();
