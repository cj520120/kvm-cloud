...
public class ConvertService {
    ...
    public StorageModel initStorageModel(StorageEntity entity) {
        ...
        if (model.getHostId() != 0) {
            model.setHost(this.initHostModel(this.hostService.getHostById(model.getHostId())));
        }
        return model;
    }
    ...
    public VolumeModel initVolumeModel(VolumeEntity volume) {
        ...
        if (model.getHostId() != 0) {
            model.setHost(this.initHostModel(this.hostService.getHostById(volume.getHostId())));
        }
        if (volume.getGuestId() != 0) {
            model.setGuest(BeanConverter.convert((volume.getGuestId()), SimpleGuestModel.class));
        }
        model.setStorage(BeanConverter.convert(this.storageService.getStorageById(volume.getStorageId()), SimpleStorageModel.class));
        if (volume.getTemplateId() > 0) {
            model.setTemplate(BeanConverter.convert(this.templateService.getTemplateById(volume.getTemplateId()), TemplateModel.class));
        }
        return model;
    }
    ...
    public NetworkModel initNetworkModel(NetworkEntity entity) {
        ...
        if (model.getBasicNetworkId() != 0) {
            model.setBasic(initNetworkModel(this.networkService.getNetworkById(model.getBasicNetworkId())));
        }
        return model;
    }
