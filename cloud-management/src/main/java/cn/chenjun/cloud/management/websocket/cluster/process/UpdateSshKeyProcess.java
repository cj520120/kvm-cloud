...
public class UpdateSshKeyProcess extends AbstractClusterMessageProcess {
    ...
    public void process(NotifyData<?> msg) {
        ...
        NotifyData<ResultUtil<SshAuthorizedModel>> sendMsg = NotifyData.<ResultUtil<SshAuthorizedModel>>builder().id(msg.getId()).type(Constant.NotifyType.UPDATE_SSH_KEY).data(resultUtil).version(System.currentTimeMillis()).build();
