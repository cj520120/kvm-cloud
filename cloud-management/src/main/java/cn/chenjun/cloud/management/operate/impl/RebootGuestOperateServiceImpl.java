...
@Component
@Slf4j
public class RebootGuestOperateServiceImpl extends AbstractOperateService<RebootGuestOperate, ResultUtil<Void>> {


    @Override
    public void operate(RebootGuestOperate param) {
        GuestEntity guest = guestDao.findById(param.getGuestId());
        HostEntity host = hostDao.findById(guest.getLastHostId());
    ...
    public void onFinish(RebootGuestOperate param, ResultUtil<Void> resultUtil) {
        ...
        NotifyContextHolderUtil.append(NotifyData.<ResultUtil<GuestEntity>>builder().id(param.getGuestId()).type(Constant.NotifyType.GUEST_RESTART_CALLBACK_NOTIFY).data(ResultUtil.<GuestEntity>builder().code(resultUtil.getCode()).message(resultUtil.getMessage()).data(guest).build()).build());
