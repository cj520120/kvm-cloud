...
# `snapctl get services` returns a.js JSON array, example:
#{
#"n6801": {
#   "listen": 6801,
#   "vnc": "localhost:5901"
...
snapctl get services | jq -c '.[]' | while read service; do # for each service the user sepcified..
  ...
  if [ ! $listen_port_valid ] || [ -z "$vnc_host_port" ]; then
    # invalid values mean the service is disabled, do nothing except for printing a.js message (logged in /var/log/system or systemd journal)
